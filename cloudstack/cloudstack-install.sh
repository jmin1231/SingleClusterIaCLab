...
# State (run log + tracker) is anchored to the repo root via this script's path, not
# $PWD; override CS_STATE_DIR to relocate. Why not $PWD: see docs/notes.md.
CS_STATE_DIR="${CS_STATE_DIR:-$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/../.." && pwd)}"
CS_LOGFILE="${CS_LOGFILE:-$CS_STATE_DIR/installer.log}"
TRACKER_FILE="${TRACKER_FILE:-$CS_STATE_DIR/cloudstack-installer-tracker.conf}"
mkdir -p "$CS_STATE_DIR" 2>/dev/null || true
...
# Unattended all-in-one: when 1, drive install + zone deploy from env/defaults and
# force silent mode up front (CLOUDSTACK_UNATTENDED: see docs/notes.md).
UNATTENDED="${CLOUDSTACK_UNATTENDED:-0}"
if [[ "$UNATTENDED" == "1" ]]; then
  PROMPT=0
fi

# Root password of the KVM host, reused as the "add host" SSH password during
# unattended zone deployment (mirrors ROOT_PASSWORD in cloudstack-install-all.sh).
...
# Shadow the `dialog` binary so silent/headless runs never depend on a terminal
# (why, and the is_silent branches it forces: see docs/notes.md).
dialog() {
  if is_silent; then
    # Only consume stdin when it is a pipe (a gauge producer); reading a real
    # terminal here would block on a standalone dialog.
...
show_dialog() {
  ...
  # Headless: print instead of rendering an ncurses box so messages still reach the log.
  if is_silent; then
    printf '[%s] %b\n' "$title" "$msg"
    return 0
  fi
...
_configure_rpm_repo() {
  ...
  {
    echo "Adding CloudStack repository..."
    if cat >/etc/yum.repos.d/cloudstack.repo <<EOF; then
[cloudstack]
name=CloudStack
baseurl=$repo_entry
enabled=1
...
# Echo the id of an existing CloudStack resource (or empty) so a re-run adopts what a
# prior partial run created instead of colliding on "already exists" (see docs/notes.md).
# Usage: lookup_resource_id <jq-array-path> <cmk list args...>
lookup_resource_id() {
  local jq_path="$1"
  shift
  cmk list "$@" 2>/dev/null | jq -r "${jq_path} // empty" 2>/dev/null || true
}
...
configure_management_server_database() {
  ...
  # Idempotency: skip re-deploy if the cloud DB already exists, detected independently
  # of the step tracker (why: see docs/notes.md).
  if mysql -u"$db_user" -p"$db_pass" -e "USE cloud; SELECT 1 FROM version LIMIT 1;" &>/dev/null; then
    log "CloudStack database already deployed; skipping deployment."
    show_dialog "info" "$title" "CloudStack database already deployed.\n\nSkipping deployment."
    set_tracker_field "$tracker_key" "yes"
    return 0
  fi
...
wait_for_management_server() {
  ...
  # Headless: poll directly and return a real status (the gauge pipeline masks a timeout).
  if is_silent; then
    while [ $elapsed -lt $timeout ]; do
      local status_code=$(curl -s -o /dev/null -w "%{http_code}" "$url")
      if [[ "$status_code" == "200" || "$status_code" == "401" ]]; then
        log "Management Server is ready (HTTP $status_code)."
        return 0
      fi
      sleep $interval
      elapsed=$((elapsed + interval))
    done
    log "Timeout waiting for Management Server after ${timeout}s."
    return 1
  fi
...
check_cloudmonkey_availability() {
  # Headless: run the checks directly so the real status propagates past the dialog shadow.
  if is_silent; then
    if ! command -v cmk &>/dev/null; then
      log "CloudMonkey (cmk) not found."
      return 1
    fi
    if ! cmk sync &>/dev/null; then
      log "Failed to initialize CloudMonkey (cmk sync)."
      return 1
    fi
    log "CloudMonkey is ready."
    return 0
  fi
...
deploy_zone() {
  ...
  local root_pass
  if is_silent; then
    # Unattended: use the shared ROOT_PASSWORD and verify SSH once instead of prompting.
    root_pass="${ROOT_PASSWORD:-}"
    [[ -n "$root_pass" ]] || error_exit "ROOT_PASSWORD must be set for unattended zone deployment."
    if ! sshpass -p "$root_pass" ssh -o StrictHostKeyChecking=no \
      -o ConnectTimeout=5 \
      -o LogLevel=ERROR \
      -o UserKnownHostsFile=/dev/null \
      root@"$KVM_HOST_IP" "echo 2>/dev/null"; then
      error_exit "Failed to SSH to KVM host $KVM_HOST_IP with the provided ROOT_PASSWORD."
    fi
    log "Verified root SSH to KVM host $KVM_HOST_IP."
  else
    while true; do
  ...
  if is_silent; then
    # Unattended: accept the computed defaults (values are the odd entries of
    # the label/value `defaults` array) instead of prompting via the form.
    results=()
    local d=1
    while [[ $d -lt ${#defaults[@]} ]]; do
      results+=("${defaults[d]}")
      ((d += 2))
    done
  else
  ...
  # Idempotency: if the zone is already deployed and Enabled, record it and skip
  # re-deploy so create steps don't collide on "already exists" (see docs/notes.md).
  local existing_zone_state
  existing_zone_state=$(cmk list zones name="$zone_name" 2>/dev/null | jq -r '.zone[0].allocationstate // empty')
  if [[ "$existing_zone_state" == "Enabled" ]]; then
    log "Zone '$zone_name' already deployed and Enabled; recording tracker and skipping."
    show_dialog "info" "$title" "Zone '$zone_name' already exists and is Enabled.\n\nSkipping zone deployment."
    set_tracker_field "$tracker_key" "yes"
    return 0
  fi
  ...
  local cluster_id=""
  {
    ...
    [[ -z "$zone_id" ]] && zone_id=$(lookup_resource_id '.zone[0].id' zones name="$zone_name")
    if [[ -n "$zone_id" ]]; then
      log "Reusing existing zone: $zone_id"
    else
    ...
    [[ -z "$physicalnetwork_id" ]] && physicalnetwork_id=$(lookup_resource_id '.physicalnetwork[0].id' physicalnetworks name="$phy_name" zoneid="$zone_id")
    if [[ -n "$physicalnetwork_id" ]]; then
      log "Reusing existing physical network: $physicalnetwork_id"
    else
    ...
    # Ensure traffic types exist; run unconditionally, `|| true` ignoring the
    # already-exists error so a re-run is idempotent (HTTP 530: see docs/notes.md).
    update_progress_bar "30" "# Adding Traffic Types..."
    cmk add traffictype traffictype=Management physicalnetworkid="$physicalnetwork_id" >/dev/null 2>&1 || true
    cmk add traffictype traffictype=Guest physicalnetworkid="$physicalnetwork_id" >/dev/null 2>&1 || true
    cmk add traffictype traffictype=Public physicalnetworkid="$physicalnetwork_id" >/dev/null 2>&1 || true
    ...
    if [[ -n "$vlan_ip_range_id" ]]; then
      ...
      # Tolerate an already-exists/overlap error on re-run instead of adopt-before-create
      # (no working `list` verb for VLAN IP ranges here: see docs/notes.md).
      local _vlan_out
      _vlan_out=$(cmk create vlaniprange \
        zoneid="$zone_id" \
        vlan=untagged \
        gateway="$GATEWAY" \
        netmask="$NETMASK" \
        ...
        endip="$public_end" \
        forvirtualnetwork=true 2>&1) || true
      vlan_ip_range_id=$(echo "$_vlan_out" | jq -r '.vlan.id // empty' 2>/dev/null || true)
      if [[ -z "$vlan_ip_range_id" ]] && ! echo "$_vlan_out" | grep -qiE 'already|overlap|exist'; then
        cmk_api_fail_dialog_msg "Failed to add Public IP range"
      fi
      [[ -n "$vlan_ip_range_id" ]] && set_tracker_field "vlan_ip_range_id" "$vlan_ip_range_id"
    fi
    ...
    [[ -z "$pod_id" ]] && pod_id=$(lookup_resource_id '.pod[0].id' pods name="$pod_name" zoneid="$zone_id")
    if [[ -n "$pod_id" ]]; then
      log "Reusing existing pod: $pod_id"
    else
    ...
    [[ -z "$cluster_id" ]] && cluster_id=$(lookup_resource_id '.cluster[0].id' clusters name="$cluster_name" zoneid="$zone_id")
    if [[ -n "$cluster_id" ]]; then
      log "Reusing existing cluster: $cluster_id"
    else
    ...
    [[ -z "$primary_storage_id" ]] && primary_storage_id=$(lookup_resource_id '.storagepool[0].id' storagepools name="$primary_name" zoneid="$zone_id")
    if [[ -n "$primary_storage_id" ]]; then
      log "Reusing existing primary storage: $primary_storage_id"
    else
    ...
    [[ -z "$secondary_storage_id" ]] && secondary_storage_id=$(lookup_resource_id '.imagestore[0].id' imagestores name="$secondary_name")
    if [[ -n "$secondary_storage_id" ]]; then
      log "Reusing existing secondary storage: $secondary_storage_id"
    else
...
main() {
  ...
  if is_unattended; then
    # No menu: force silent mode and install the full all-in-one set; the block below deploys the zone.
    log "Unattended mode: performing all-in-one installation."
    PROMPT=0
    SELECTED_COMPONENTS=("nfs" "management" "agent" "usage")
    custom_install
  else
