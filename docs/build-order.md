...
**Choose the CNI here, because you cannot change it later without rebuilding.** k3s ships flannel
plus a kube-router-based NetworkPolicy controller, and that combination *enforces* NetworkPolicy
correctly out of the box. What it does not do is show you anything: a denied packet produces no
event, no log line and no counter, so a policy that is wrong and a policy that is working look
identical. Since 9.6 exists to teach segmentation, that is the wrong default here.

Install with `--flannel-backend=none --disable-network-policy`, then **Cilium**. Expect the node to
sit `NotReady` until the CNI is up — that is the CNI being genuinely responsible for pod networking,
not a fault.

**The alternative, and the budget question it raises.** Calico is the other honest answer: mature,
lighter per node, and `GlobalNetworkPolicy` plus rule-level logging covers most of what you want.
Cilium buys **Hubble** — a live flow view where you watch the drop happen — which is the single
thing that turns 9.6 from "the manifest applied" into "I saw it refuse". It also costs more memory
per node, and 9.2's agents are sized at ~1 GB. Measure the agent's footprint immediately after
install and put the number in the budget; if it does not fit, take Calico, and if that does not fit
either, go back to k3s's built-in controller and accept that 9.6 becomes a test-by-curl exercise.
See 0.5 rule 3 — the ceiling is the ceiling.

**Done when:** `kubectl get nodes` works and a test pod reaches Running, at a version you chose,
with `cilium status` (or `calicoctl node status`) healthy and the per-node memory cost written down.
...
### 9.6 Segment the cluster, deny by default · `M`

The Phase 7.2 exercise, one layer up. Write a default-deny NetworkPolicy for the application
namespace, then allow exactly the paths Phase 12 needs: web → api, api → Postgres. Nothing else.

**Learning:** NetworkPolicy is **allow-only and additive**. There is no deny rule — you remove
traffic by selecting pods with a policy that permits nothing, and every other policy that selects
the same pod *unions* into it. That is the opposite of how a CloudStack ACL list reads, where rules
are ordered and a later one can deny what an earlier one allowed. Both guard the same three tiers;
they do not express it the same way, and the day they disagree the packet is refused by whichever
sits closer to it.

**Three traps, in the order you will hit them:**

- **Egress deny breaks DNS first.** The moment a default-deny egress policy applies, pods stop
  resolving anything, because kube-dns is now on the far side of it. Every symptom looks like an
  application bug. Allow UDP and TCP 53 to the kube-system DNS pods before you debug anything else.
- **A policy that selects no pods is a silent no-op.** A typo in `podSelector` produces a policy
  that applies successfully, appears in `kubectl get netpol`, and protects nothing.
- **`podSelector: {}` selects every pod in the namespace, not none.** That empty selector is how a
  default-deny is written, and it is also how an accidental cluster-wide allow gets written.

**Then prove it, rather than assuming it.** Exec into the web pod and open a connection to Postgres
directly. It must fail. With Cilium, watch it fail in `hubble observe --verdict DROPPED` — seeing
the verdict is the whole reason 9.1 chose the CNI it did. With Calico, enable rule logging and read
it in the node's syslog.

**Done when:** web reaches api, api reaches Postgres, web cannot reach Postgres, DNS still works
from every pod, and you have watched the denied flow rather than inferred it from a timeout.
...
>
> **Drill 9b** — delete the default-deny policy and re-run the Postgres connection from web. It
> should succeed. Nothing warns you that segmentation is gone; the only signal is the test you
> just wrote.
